...
class BannerModel {
  ...
  constructor(parent, data) {
    ...
    injectProperties(this, {
      ...
      /**
       * Через сколько секунд баннер считается устаревшим.
       *
       * @type {number}
       */
      expired: 0,
  ...
  loadDBData(data) {
    return new Promise((resolve, reject) => {
      if (data && typeof data === "object") {
        ...
        this.expired = undefined === data.expired ? 0 : parseInt(data.expired);
        return resolve();
      } else {
        return reject();
      }
  ...
  isActual() {
    ...
    // Баннер уже в показе
    // ИЛИ
    // Баннер считан И время резервирования еще не прошло
    return this.started || (
      this.loaded > 0 && (
        time - this.loaded - this.expired - this.time * 2 > 0
      ));
  }
  ...
  parseTemplate($this, template) {
    let dbFields = [
      "adid", "type", "price", "timeLeft", "link", "file", "size", "alt_text", "linkClass", "key", "serverUrl"
    ];
    let val = "";
    for (let i in dbFields) {
      if ("serverUrl" === dbFields[i]) {
        val = serverUrl();
      } else if ("key" === dbFields[i]) {
        val = escape($this.key);
      } else {
        val = $this[dbFields[i]];
      }
...
class Banners {
  ...
  start() {
    ...
    if (this.current) {
      if (this.current.isActual()) {
        this.current.start();
        this.tickHandler = setInterval(function () {
          $this.tick();
        }, 1000);
      } else {
        log && console.log("Banner expired!");
        this.clearCurrent();
        this.nextRun();
      }
    } else {
      log && console.log("No banners available!");
    }
  }
  ...
  nextBanner() {
    if (this.getNextBannerEnabled()) {
      ...
      ajax(
        ...
        data,
        function (response) {
          console.log("nextBanner()", response);
          // Добавление баннера
          $this.add(response);
